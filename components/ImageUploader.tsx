        ...
        {!preview ? (
          ...
          <div className="relative group w-full h-full rounded-lg overflow-hidden">
            ...
            <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-lg">
               <button onClick={(e) => { e.stopPropagation(); openModal(preview); }} className="bg-sky-600 hover:bg-sky-700 text-white font-semibold px-4 py-2 rounded-md text-sm mr-2 shadow-md transition-all duration-200">View</button>
               <button onClick={(e) => { e.stopPropagation(); handleRemoveImage(); }} className="bg-red-600 hover:bg-red-700 text-white font-semibold px-4 py-2 rounded-md text-sm shadow-md transition-all duration-200">Remove</button>
            </div>
          </div>
        )}
      </div>
